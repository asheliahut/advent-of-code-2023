...
for (let i = 0; i < map.length; i++) {
  for (let j = 0; j < map[i].length; j++) {
    if (map[i][j] === "*") {
      // check for 2 adjacent numbers, set default to 1 for multiplication
      let adjacentNumbers: number[] = [];

      // left
      if (isNumber(map[i][j - 1])) {
        adjacentNumbers.push(getFullNumber(map, i, j - 1));
      }
      // right
      if (isNumber(map[i][j + 1])) {
        adjacentNumbers.push(getFullNumber(map, i, j + 1));
      ...
      if (isNumber(map[i - 1][j])) {
        adjacentNumbers.push(getFullNumber(map, i - 1, j));
        doesUpHaveNumber = true;
      }
      // up left
      if (!doesUpHaveNumber && isNumber(map[i - 1][j - 1])) {
        adjacentNumbers.push(getFullNumber(map, i - 1, j - 1));
      }
      // up right
      if (!doesUpHaveNumber && isNumber(map[i - 1][j + 1])) {
        adjacentNumbers.push(getFullNumber(map, i - 1, j + 1));
      ...
      if (adjacentNumbers.length === 2) {
        numbersSum += adjacentNumbers[0] * adjacentNumbers[1];
      }
    }
  }
}
