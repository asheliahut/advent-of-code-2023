...
for (const pattern of patterns) {
  const [rowReflectionIdxs, colReflectionIdxs] = findReflections(pattern);

  for (const rowReflectionIdx of rowReflectionIdxs) {
    total += (rowReflectionIdx + 1) * 100;
  }

  for (const colReflectionIdx of colReflectionIdxs) {
    total += colReflectionIdx + 1;
  }
}
